...
--
-- $Revision: 14$        $Date: 2/27/2008 3:17:06 PM$
--
--
-- Description:  <ADD>
--
...
-- Remove the following lines that are commented from this code after 02-15-2004

--update "&1".ip_business_factors set(low,med,high) =
--(select .026373,.046042,.065712 from dual) where type='Dosing';
--update "&1".ip_business_factors set(low,med,high) =
--(select .026373,.046042,.065721 from dual) where type='Dosing';
--update "&1".ip_business_factors set(low,med,high) =
...
--Following lines were added by Debashish on 04-14-2004

update "&1".ip_business_factors set(low,med,high) =
(select low,med,high from tsm_stage.ip_business_factor_3 where type='Dosing') 
where type='Dosing';

update "&1".ip_business_factors set(low,med,high) =
(select low,med,high from tsm_stage.ip_business_factor_3 where type='Country') 
where type='Country';

update "&1".ip_business_factors set(low,med,high) =
(select low,med,high from tsm_stage.ip_business_factor_3 where type='Study') 
where type='Study';

update "&1".ip_business_factors set(low,med,high) =
(select low,med,high from tsm_stage.ip_business_factor_3 where type='Population') 
where type='Populate';

update "&1".ip_business_factors set(low,med,high) =
(select low,med,high from tsm_stage.ip_business_factor_3 where type='Duration') 
where type='Ph1dur';

update "&1".ip_business_factors set(low,med,high) =
(select low,med,high from tsm_stage.ip_business_factor_3 where type='Confine') 
where type='Confine';
...
declare
 ...
 ibf_maxid number(10);
 ibf_exists number(3);
 invalid_intercept exception;
 check_tsm_stage number(3);
 invalid_stage_data exception;

begin
 
 select count(*) into check_tsm_stage from tsm_stage.ip_business_factor_3 
 where type='Intercept';

 if check_tsm_stage <>1 then
  raise invalid_stage_data;
 end if;
 
 select nvl(max(id),0)+1 into ibf_maxid from "&1".ip_business_factors;
 select count(*) into ibf_exists from "&1".ip_business_factors where type='Intercept';

 if ibf_exists=0 then
   Insert into "&1".ip_business_factors(ID,TYPE,FACTOR,low,med,high)
   select ibf_maxid,'Intercept',med,low,med,high from tsm_stage.ip_business_factor_3
   where type='Intercept';
 elsif ibf_exists=1 then
   update "&1".ip_business_factors set (FACTOR,low,med,high)=
   (select med,low,med,high from tsm_stage.ip_business_factor_3 where type='Intercept')
   where type='Intercept';
 else
   raise invalid_intercept;
 end if;
...
exception
   ...
   when invalid_intercept then
	raise_application_error(-20224,'Multiple intercept rows in ip_business_factors');
   when invalid_stage_data then
	raise_application_error(-20225,'Multiple intercept rows in tsm_stage');
...
--
-- $Log: 
--  14   DevTSM    1.13        2/27/2008 3:17:06 PM Debashish Mishra  
--  13   DevTSM    1.12        2/7/2007 10:28:02 PM Debashish Mishra  
--  12   DevTSM    1.11        3/3/2005 6:40:25 AM  Debashish Mishra  
--  11   DevTSM    1.10        5/7/2004 11:29:49 AM Debashish Mishra  
--  10   DevTSM    1.9         5/6/2004 8:15:07 PM  Debashish Mishra  
--  9    DevTSM    1.8         4/14/2004 5:23:14 PM Debashish Mishra Added new code
--       to update the low, med,high values from a foxpro factor table instead of
--       hardcoded values
--  8    DevTSM    1.7         1/30/2004 3:26:34 PM Debashish Mishra  
--  7    DevTSM    1.6         8/29/2003 5:13:13 PM Debashish Mishra  
--  6    DevTSM    1.5         9/11/2002 10:35:10 AMDebashish Mishra chnages to
--       ip_business_factors for num_days for ph2+dur type
--  5    DevTSM    1.4         8/30/2002 12:43:08 PMDebashish Mishra Modified for
--       passing the schema name as an parameter
--  4    DevTSM    1.3         5/15/2002 9:45:48 AM Debashish Mishra  
--  3    DevTSM    1.2         4/17/2002 3:48:18 PM Debashish Mishra  
