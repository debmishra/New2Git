...
-- Following lines were added as per the request of Kelly on 09-13-2004
-- These lines should be uncommented only after releasing GM1.3 in production 

--declare

--popti_maxid number(10);
--zero_exists number(1);
--cursor c1 is select id from "&1".pap_odc_pct where country_id is not null;

--begin

--select nvl(max(id),0)+1 into popti_maxid from
--"&1".pap_odc_pct_to_indmap;
-- for ix1 in c1 loop

--  select count(*) into zero_exists from "&1".pap_odc_pct_to_indmap
--  where indmap_id=0 and pap_odc_pct_id=ix1.id;

--  If zero_exists=0 then
--    Insert into "&1".pap_odc_pct_to_indmap values(popti_maxid,ix1.id,0,0);
--    popti_maxId:=popti_maxid+1;
--  end if;
...
--  5    DevTSM    1.4         9/14/2004 7:01:24 AM Debashish Mishra  
--  4    DevTSM    1.3         8/29/2003 5:13:28 PM Debashish Mishra  
--  3    DevTSM    1.2         5/6/2003 9:36:35 AM  Debashish Mishra Fixed the
--       spelling mistake in ophthalmology
--  2    DevTSM    1.1         8/30/2002 12:43:12 PMDebashish Mishra Modified for
